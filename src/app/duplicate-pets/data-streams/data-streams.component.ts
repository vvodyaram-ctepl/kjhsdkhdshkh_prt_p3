...
export class DataStreamsComponent implements OnInit {
  ...
  stitch() {
    if (this.selectedPetIds.length > 0) {
      ...
      this.toastr.error('Please select atleast one pet', 'Error!');
    }
  }
  ...
  onSubmit() {
    ...
    this.streamForm.value.arr.forEach((element, index) => {
      ...
      if (primaryPetArray.length) {
        ...
        // Only one primary pet selected
        if (primaryPetArray.length == 1 && primaryPetArray.length == element.length) {
          let obj = Object.assign({});
          ...
          //Multiple primary pets available.
          let isOnlyPrimaryPet = (primaryPetArray.length == element.length) ? true : false;
          if (isOnlyPrimaryPet) {
            for (let i = 0; i < primaryPetArray.length; i++) {
              let obj = Object.assign({});
            ...
            for (let i = 1; i < primaryPetArray.length; i++) {
              let obj = Object.assign({});
